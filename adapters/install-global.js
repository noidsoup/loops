...
/**
 * True if an existing path is a loops artifact we may refresh.
 * Never treat an arbitrary foreign file/symlink as replaceable just because
 * forceManaged was requested — that violated the collision policy.
 */
function isLoopsOwnedArtifact(targetPath, expectedSrc) {
  if (isManagedTarget(targetPath, expectedSrc)) return true;
  if (isSymlink(targetPath)) {
    // Prefixed symlink left from an older install (possibly relative / moved).
    // Only replace if it still resolves under this repo.
    const cur = resolveMaybe(targetPath);
    const repoRoot = resolveMaybe(REPO);
    return cur === repoRoot || cur.startsWith(repoRoot + path.sep);
  }
  ...
  try {
    ...
    return (
      text.includes('Auto-generated by loops') ||
      text.includes(LOOPS_MARKER) ||
      text.includes('Loop pack disambiguation') ||
...
function linkOrCopy(src, dest, { forceManaged = false } = {}) {
  ...
  if (fs.existsSync(dest) || isSymlink(dest)) {
    const replaceable = forceManaged
      ? isLoopsOwnedArtifact(dest, src)
      : isManagedTarget(dest, src);
    if (replaceable) {
 ...
 */
function isManagedClaudeSkillDir(dest, prefixedName, expectedSrc = null) {
  if (!fs.existsSync(dest) && !isSymlink(dest)) return true;
  if (isSymlink(dest)) {
    // Legacy whole-dir symlink: only managed if it points at our emit output
    // (or, without expectedSrc, somewhere under this repo).
    if (expectedSrc) return isManagedTarget(dest, expectedSrc);
...
function installClaudeSkills() {
  ...
  for (const name of listLoopNames()) {
    ...
    if (fs.existsSync(skillDest) || isSymlink(skillDest)) {
      if (!isManagedClaudeSkillDir(skillDest, prefixed, skillSrc)) {
        results.push({
...
function uninstallCursor(removed, skipped) {
  ...
  const resolutionSrc = path.join(REPO, 'INSTALL-RESOLUTION.mdc');
  const resolutionDest = path.join(CURSOR_RULES, 'loops-resolution.mdc');
  if (fs.existsSync(resolutionDest) || isSymlink(resolutionDest)) {
    if (isLoopsOwnedArtifact(resolutionDest, resolutionSrc) || path.basename(resolutionDest).startsWith(PREFIX)) {
      fs.rmSync(resolutionDest, { force: true });
      removed.push(resolutionDest);
    } else {
      skipped.push(resolutionDest);
    }
  }
...
function uninstallClaude(removed, skipped) {
  ...
  const resolutionDest = path.join(CLAUDE_RULES, 'loops-resolution.md');
  if (fs.existsSync(resolutionDest) || isSymlink(resolutionDest)) {
    const text = isSymlink(resolutionDest) ? '' : fs.readFileSync(resolutionDest, 'utf8');
    if (
      !text ||
      text.includes('Loop pack disambiguation') ||
      text.includes('loops-resolution') ||
      text.includes(LOOPS_MARKER)
    ) {
      fs.rmSync(resolutionDest, { force: true });
      removed.push(resolutionDest);
    } else {
      skipped.push(`${resolutionDest} (not a loops resolution rule)`);
    }
  }
